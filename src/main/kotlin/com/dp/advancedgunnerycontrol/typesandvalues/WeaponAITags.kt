...
fun getTagTooltip(tag: String) : String{
    ...
    return when{
        holdRegex.matches(tag) -> "Weapon will stop firing if ship flux exceeds ${extractRegexThresholdAsPercentageString(holdRegex, tag)}."
        pdFluxRegex.matches(tag) -> "Weapon will act as PD mode while ship flux > ${extractRegexThresholdAsPercentageString(pdFluxRegex, tag)}."
        avoidArmorRegex.matches(tag) -> "Weapon will fire when the shot is likely to hit shields (as TargetShields) OR a section of hull " +
                "\nwhere the armor is low enough to achieve at least ${extractRegexThresholdAsPercentageString(avoidArmorRegex, tag)} " +
        ...
        rangeRegex.matches(tag) -> "Weapon will only target and fire at targets if they are closer than ${extractRegexThresholdAsPercentageString(
            rangeRegex, tag)} of weapon range." +
                "\nThis is useful for weapons (especially missiles) with slow projectiles, such as e.g. sabots."
        forceFireRegex.matches(tag) -> "ForceFire: Weapon will ignore firing restrictions of other tags while flux < ${extractRegexThresholdAsPercentageString(
            forceFireRegex, tag)}." +
...
val tagIncompatibility = mapOf(
    "PD" to listOf("Fighter", "Opportunist", "NoPD", "PD(Flx>N%)", "BigShips", "SmallShips", "CnsrvPDAmmo", "PrioritisePD"),

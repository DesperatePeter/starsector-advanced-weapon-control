...
class PDAtAmmoThresholdAI(baseAI: AutofireAIPlugin, suffix: SuffixBase, private val threshold: Float)
    ...
    override fun isBaseAITargetValid(ship: ShipAPI?, missile: MissileAPI?): Boolean {
        if(ammoLevel(weapon) > threshold) return true
    ...
    override fun isValid(): Boolean {
        return isPD(weapon) && weapon.usesAmmo()
    }
}